...
void main() {
  ...
  vec4 color = texture(tex0, uv) + texture(tex0, uv + vec2(x, y)) + texture(tex0, uv + vec2(x, -y)) + texture(tex0, uv + vec2(-x, y)) + texture(tex0, uv + vec2(-x, -y));
  ...
  bandHsb1.z = abs(bandHsb1.z + bright1 + brightLfo1);//if i add any lfo here it kills the output. if i add sin(brightLfo1) its fine....so just manually add 1 lfo here whatever
	// update: if I comment out the foldover section that restores brightLfo to working...
